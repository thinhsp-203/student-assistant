...
from app.schemas.student import StudentResponse, TranscriptResponse, ProgressResponse
...
async def get_student(student_id: str, student_service: StudentService = Depends(get_student_service)):
...
async def get_transcript(student_id: str, student_service: StudentService = Depends(get_student_service)):
    student = await student_service.get_student(student_id)
    if not student:
        raise HTTPException(status_code=404, detail="Student not found")
...
async def get_progress(student_id: str, student_service: StudentService = Depends(get_student_service)):
